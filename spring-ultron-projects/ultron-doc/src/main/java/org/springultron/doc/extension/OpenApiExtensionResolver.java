...
import org.springultron.doc.utils.MarkdownUtils;
 ...
 * @date 2020/10/24 7:17
 */
public class OpenApiExtensionResolver {
    private final Logger logger = LoggerFactory.getLogger(OpenApiExtensionResolver.class);
    ...
    private String readCustomHome(String customHomeLocation) {
        ...
        try {
            ...
            if (resources.length > 0) {
                ...
                customHomeContent = new String(CommonUtils.readBytes(resource.getInputStream()), StandardCharsets.UTF_8);
            }
        } catch (Exception e) {
            logger.warn("(Ignores) Failed to read CustomHomeLocation Markdown files,Error Message:{} ", e.getMessage());
        }
