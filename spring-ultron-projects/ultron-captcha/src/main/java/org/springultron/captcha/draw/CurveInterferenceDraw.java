...
public enum CurveInterferenceDraw implements InterferenceDraw {
    ...
    public void draw(Graphics2D g, int width, int height, Font[] fonts, Random random) {
        ...
        int x1 = 5;
        int y1 = CaptchaUtils.randNum(random, 5, height / 2);
        int maxHeight = height - 5;
        int minWidth = width / 4;
        int maxWidth = width / 4 * 3;
        int ctrLx1 = CaptchaUtils.randNum(random, minWidth, maxWidth);
        int ctrLy1 = CaptchaUtils.randNum(random, 5, maxHeight);
        int ctrLx2 = CaptchaUtils.randNum(random, minWidth, maxWidth);
        int ctrLy2 = CaptchaUtils.randNum(random, 5, maxHeight);
        int x2 = width - 5;
        int y2 = CaptchaUtils.randNum(random, height / 2, maxHeight);
        // 画三次曲线
