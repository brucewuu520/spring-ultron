...
public class MathCaptchaDraw implements CaptchaDraw {
	...
	public String draw(Graphics2D g, int width, int height, Font[] fonts, Random random) {
		...
		// 深色
		g.setColor(CaptchaUtils.randColor(random, 0, 100));
		// 数学公式
	...
	public boolean validate(String captchaCode, String userInputCaptcha) {
		...
		for (int i = userInputCaptcha.length(); --i >= 0; ) {
			int chr = userInputCaptcha.charAt(i);
			if (chr < 48 || chr > 57) {
				return false;
			}
		}
		int captchaNum = Integer.parseInt(userInputCaptcha);
		int evalNum = Expression.eval(captchaCode);
